        ...
        apiSignature: (() => {
          ...
          for (const route of backendRoutes) {
            ...
            if (
              ...
              route.apiSignature?.requestType
            ) {
          ...
          if (returnTypes.size > 0) {
            ...
            aggregated.returnType =
              returnTypeArray.length === 1
                ? returnTypeArray[0]
                : returnTypeArray[0];
          }
          ...
          if (requestTypes.size > 0) {
            ...
            aggregated.requestType =
              requestTypeArray.length === 1
                ? requestTypeArray[0]
                : requestTypeArray[0];
          }
          ...
          if (responseTypes.size > 0) {
            ...
            aggregated.responseType =
              responseTypeArray.length === 1
                ? responseTypeArray[0]
                : responseTypeArray[0];
          }
...
function applySubmitOnlyPreset(
  ...
  if (forbidden.length > 0) {
    violations.push(
      `Submit-only contract violated: remove events [${forbidden.join(', ')}]`,
    );
  }
...
function applyNavOnlyPreset(
  ...
  if (forbidden.length > 0) {
    violations.push(
      `Nav-only contract violated: remove events [${forbidden.join(', ')}]`,
    );
  }
...
export function generateBehavioralPredictions(ast: AstExtract): string[] {
  ...
  if (
    ast.hooks.includes('useForm') ||
    ast.functions.some((f) => f.includes('validate'))
  ) {
...
export function inferDescription(filePath: string, ast: AstExtract): string {
  ...
  if (
    fileName.toLowerCase().includes('input') ||
    fileName.toLowerCase().includes('field')
  ) {
  ...
  if (
    fileName.toLowerCase().includes('nav') ||
    fileName.toLowerCase().includes('menu')
  ) {
