            ...
            if (propName === 'components') {
              ...
              if (initializer && Node.isObjectLiteralExpression(initializer)) {
                ...
                initializer.getProperties().forEach((objProp) => {
                  try {
                    if (
                      Node.isShorthandPropertyAssignment(objProp) ||
                      Node.isPropertyAssignment(objProp)
                    ) {
                    ...
                    if (Node.isPropertyAssignment(objProp)) {
                      ...
                      if (initializer) {
                        ...
                        if (Node.isObjectLiteralExpression(initializer)) {
                          ...
                          const typeNode = propObj.getProperty('type');
                          const requiredNode = propObj.getProperty('required');
                          const defaultNode = propObj.getProperty('default');

                          if (typeNode && Node.isPropertyAssignment(typeNode)) {
                            typeText =
                              typeNode.getInitializer()?.getText() || 'any';
                          }

                          // If required is false or default exists, it's optional
                          if (
                            requiredNode &&
                            Node.isPropertyAssignment(requiredNode)
                          ) {
                            const requiredValue = requiredNode
                              .getInitializer()
                              ?.getText();
                        ...
                        const normalizedType =
                          normalizeVueRuntimeType(typeText);
                        props[name] = normalizePropType(
                          normalizedType,
                          isOptional,
                        );
                      }
          ...
          if (funcName === 'defineEmits') {
            ...
            if (typeArgs.length > 0) {
              ...
              try {
                ...
                const eventMatches = typeText.matchAll(
                  /\(e:\s*['"]([^'"]+)['"]([^)]*)\)\s*:\s*void/g,
                );
