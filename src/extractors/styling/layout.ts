...
export function extractLayoutMetadata(source: SourceFile): LayoutMetadata {
  ...
  try {
    ...
    if (hasGrid) {
      ...
      const gridColsClasses = allClasses.filter((cls) =>
    ...
    if (hasGrid) {
      const hasCardClasses = coreClasses.some(
        (c) => /card/.test(c) || /rounded/.test(c) || /shadow/.test(c),
      );
      if (hasCardClasses) {
...
export function extractVisualMetadata(source: SourceFile): VisualMetadata {
  ...
  try {
    ...
    const colorClasses = coreClasses.filter((c) =>
      /^(?:bg-|text-|border-)/.test(c),
    );
    ...
    const spacingClasses = coreClasses.filter((c) =>
      /^-?(?:p-|m-|px-|py-|mx-|my-|pt-|pb-|pl-|pr-|mt-|mb-|ml-|mr-)[\w.[\]]+/.test(
        c,
      ),
    );
    ...
    if (radiusClasses.length > 0) {
      ...
      const sortedByFreq = Array.from(radiusFreq.entries()).sort(
    ...
    const typographyClasses = coreClasses.filter((c) =>
      /^(?:text-|font-)/.test(c),
    );
    if (typographyClasses.length > 0) {
      visual.typography = Array.from(new Set(typographyClasses))
        .sort()
        .slice(0, 10);
    }
  } catch (error) {
