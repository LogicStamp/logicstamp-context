 ...
 */
const ANT_PACKAGE_PATTERNS = [/^antd$/, /^@ant-design\//];
  ...
  try {
    ...
    try {
      importDeclarations.forEach((imp) => {
        ...
        const isAntPackage =
          ANT_PACKAGE_PATTERNS.some((pattern) =>
          ...
          if (
            ...
            if (ANT_COMPONENTS.includes(pascalCase)) {
              ...
              namedImports.forEach((namedImport) => {
                ...
                componentCounts.set(
                  pascalCase,
                  (componentCounts.get(pascalCase) ?? 0) + 1,
                );
    ...
    try {
      usesConfigProvider =
        ...
        (source.getDescendantsOfKind(SyntaxKind.JsxElement).some((element) => {
          ...
          const baseTag = tagName.includes('.')
            ? tagName.split('.')[0]
            : tagName;
          return (
            localToAnt.get(baseTag) === 'ConfigProvider' ||
            baseTag === 'ConfigProvider'
          );
        }) ||
          importDeclarations.some((imp) => {
            return imp
              .getNamedImports()
              .some((n) => n.getName() === 'ConfigProvider');
          }));
    } catch (error) {
      debugError('antd', 'extractAntDesign', {
        error: error instanceof Error ? error.message : String(error),
      ...
      usesIcons =
        importDeclarations.some((imp) => {
          ...
          return (
            ...
            moduleSpecifier.startsWith('@ant-design/icons/')
          );
        }) ||
        source
          .getDescendantsOfKind(SyntaxKind.PropertyAccessExpression)
