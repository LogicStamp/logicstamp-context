 ...
 */
export function getContractFromBundles(
  ...
  for (const bundle of bundles) {
    for (const node of bundle.graph.nodes) {
      if (
        normalizeEntryId(node.contract.entryId).toLowerCase() === normalizedId
      ) {
...
export function compareContracts(
  oldContract: UIFContract,
  newContract: UIFContract,
): ContractDiff {
  ...
  const compareArrays = (
    oldArr: string[],
    newArr: string[],
    diffObj: { added: string[]; removed: string[] },
  ) => {
  ...
  compareArrays(
    oldContract.composition.hooks || [],
    newContract.composition.hooks || [],
    diff.hooks,
  );
  compareArrays(
    oldContract.composition.components || [],
    newContract.composition.components || [],
    diff.components,
  );
  compareArrays(
    oldContract.composition.variables || [],
    newContract.composition.variables || [],
    diff.variables,
  );
  compareArrays(
    oldContract.composition.functions || [],
    newContract.composition.functions || [],
    diff.functions,
  );
 ...
 */
function indexContracts(bundles: LogicStampBundle[]): Map<
  string,
  {
    semanticHash: string;
    fileHash: string;
...
> {
  const m = new Map<
    string,
    { semanticHash: string; fileHash: string; entryId: string }
  >();
...
function indexBundles(
  bundles: LogicStampBundle[],
): Map<string, { bundleHash: string; entryId: string }> {
    ...
    if (oldContract) {
      ...
      if (oldContract.semanticHash !== newContract.semanticHash) {
        changes.semanticHash = {
          old: oldContract.semanticHash,
          new: newContract.semanticHash,
        };

        // Get full contracts for detailed diff
        const oldContractFull = getContractFromBundles(
          oldBundles,
          newContract.entryId,
        );
        const newContractFull = getContractFromBundles(
          newBundles,
          newContract.entryId,
        );
        if (oldContractFull && newContractFull) {
          changes.contractDiff = compareContracts(
            oldContractFull,
            newContractFull,
          );
        }
      ...
      if (oldContract.fileHash !== newContract.fileHash) {
        changes.fileHash = {
          old: oldContract.fileHash,
          new: newContract.fileHash,
        };
      }
    ...
    changed.forEach(({ entryId, semanticHash, fileHash, contractDiff }) => {
      ...
      if (contractDiff) {
        ...
        if (contractDiff.props.added.length > 0) {
          details.push(
            `Added props: ${contractDiff.props.added.map((p) => `\`${p}\``).join(', ')}`,
          );
        }
        if (contractDiff.props.removed.length > 0) {
          details.push(
            `Removed props: ${contractDiff.props.removed.map((p) => `\`${p}\``).join(', ')}`,
          );
        }
        if (contractDiff.props.changed.length > 0) {
          details.push(
            `Changed props: ${contractDiff.props.changed.map((p) => `\`${p.name}\``).join(', ')}`,
          );
        }
        ...
        if (contractDiff.emits.added.length > 0) {
          details.push(
            `Added events: ${contractDiff.emits.added.map((e) => `\`${e}\``).join(', ')}`,
          );
        }
        if (contractDiff.emits.removed.length > 0) {
          details.push(
            `Removed events: ${contractDiff.emits.removed.map((e) => `\`${e}\``).join(', ')}`,
          );
        }
        if (contractDiff.emits.changed.length > 0) {
          details.push(
            `Changed events: ${contractDiff.emits.changed.map((e) => `\`${e.name}\``).join(', ')}`,
          );
        }
        ...
        if (contractDiff.state.added.length > 0) {
          details.push(
            `Added state: ${contractDiff.state.added.map((s) => `\`${s}\``).join(', ')}`,
          );
        }
        if (contractDiff.state.removed.length > 0) {
          details.push(
            `Removed state: ${contractDiff.state.removed.map((s) => `\`${s}\``).join(', ')}`,
          );
        }
        if (contractDiff.state.changed.length > 0) {
          details.push(
            `Changed state: ${contractDiff.state.changed.map((s) => `\`${s.name}\``).join(', ')}`,
          );
        }
        ...
        if (contractDiff.hooks.added.length > 0) {
          details.push(
            `Added hooks: ${contractDiff.hooks.added.map((h) => `\`${h}\``).join(', ')}`,
          );
        }
        if (contractDiff.hooks.removed.length > 0) {
          details.push(
            `Removed hooks: ${contractDiff.hooks.removed.map((h) => `\`${h}\``).join(', ')}`,
          );
        }
        ...
        if (contractDiff.components.added.length > 0) {
          details.push(
            `Added components: ${contractDiff.components.added.map((c) => `\`${c}\``).join(', ')}`,
          );
        }
        if (contractDiff.components.removed.length > 0) {
          details.push(
            `Removed components: ${contractDiff.components.removed.map((c) => `\`${c}\``).join(', ')}`,
          );
        }
        ...
        if (contractDiff.variables.added.length > 0) {
          details.push(
            `Added variables: ${contractDiff.variables.added.map((v) => `\`${v}\``).join(', ')}`,
          );
        }
        if (contractDiff.variables.removed.length > 0) {
          details.push(
            `Removed variables: ${contractDiff.variables.removed.map((v) => `\`${v}\``).join(', ')}`,
          );
        }
        ...
        if (contractDiff.functions.added.length > 0) {
          details.push(
            `Added functions: ${contractDiff.functions.added.map((f) => `\`${f}\``).join(', ')}`,
          );
        }
        if (contractDiff.functions.removed.length > 0) {
          details.push(
            `Removed functions: ${contractDiff.functions.removed.map((f) => `\`${f}\``).join(', ')}`,
          );
        }
  ...
  if (isDebug) {
    ...
    if (changed.length > 0) {
      ...
      changed.forEach(({ entryId, semanticHash, fileHash, contractDiff }) => {
        ...
        if (semanticHash) {
          console.log(
            `    semanticHash (API/logic): ${semanticHash.old.substring(0, 12)}... → ${semanticHash.new.substring(0, 12)}...`,
          );
          console.log(
          ...
          if (contractDiff) {
            const hasChanges =
              contractDiff.props.added.length > 0 ||
              contractDiff.props.removed.length > 0 ||
              contractDiff.props.changed.length > 0 ||
              contractDiff.emits.added.length > 0 ||
              contractDiff.emits.removed.length > 0 ||
              contractDiff.emits.changed.length > 0 ||
              contractDiff.state.added.length > 0 ||
              contractDiff.state.removed.length > 0 ||
              contractDiff.state.changed.length > 0 ||
              contractDiff.hooks.added.length > 0 ||
              contractDiff.hooks.removed.length > 0 ||
              contractDiff.components.added.length > 0 ||
              contractDiff.components.removed.length > 0 ||
              contractDiff.variables.added.length > 0 ||
              contractDiff.variables.removed.length > 0 ||
              contractDiff.functions.added.length > 0 ||
              contractDiff.functions.removed.length > 0;

            if (hasChanges) {
              console.log(`    Detailed changes:`);
              if (contractDiff.props.added.length > 0) {
                console.log(
                  `      + Props: ${contractDiff.props.added.join(', ')}`,
                );
              }
              if (contractDiff.props.removed.length > 0) {
                console.log(
                  `      - Props: ${contractDiff.props.removed.join(', ')}`,
                );
              }
              if (contractDiff.props.changed.length > 0) {
                console.log(
                  `      ~ Props: ${contractDiff.props.changed.map((p) => p.name).join(', ')}`,
                );
              }
              if (contractDiff.emits.added.length > 0) {
                console.log(
                  `      + Events: ${contractDiff.emits.added.join(', ')}`,
                );
              }
              if (contractDiff.emits.removed.length > 0) {
                console.log(
                  `      - Events: ${contractDiff.emits.removed.join(', ')}`,
                );
              }
              if (contractDiff.state.added.length > 0) {
                console.log(
                  `      + State: ${contractDiff.state.added.join(', ')}`,
                );
              }
              if (contractDiff.state.removed.length > 0) {
                console.log(
                  `      - State: ${contractDiff.state.removed.join(', ')}`,
                );
              }
              if (contractDiff.hooks.added.length > 0) {
                console.log(
                  `      + Hooks: ${contractDiff.hooks.added.join(', ')}`,
                );
              }
              if (contractDiff.hooks.removed.length > 0) {
                console.log(
                  `      - Hooks: ${contractDiff.hooks.removed.join(', ')}`,
                );
              }
              if (contractDiff.components.added.length > 0) {
                console.log(
                  `      + Components: ${contractDiff.components.added.join(', ')}`,
                );
              }
              if (contractDiff.components.removed.length > 0) {
                console.log(
                  `      - Components: ${contractDiff.components.removed.join(', ')}`,
                );
              }
              if (contractDiff.variables.added.length > 0) {
                console.log(
                  `      + Variables: ${contractDiff.variables.added.join(', ')}`,
                );
              }
              if (contractDiff.variables.removed.length > 0) {
                console.log(
                  `      - Variables: ${contractDiff.variables.removed.join(', ')}`,
                );
              }
              if (contractDiff.functions.added.length > 0) {
                console.log(
                  `      + Functions: ${contractDiff.functions.added.join(', ')}`,
                );
              }
              if (contractDiff.functions.removed.length > 0) {
                console.log(
                  `      - Functions: ${contractDiff.functions.removed.join(', ')}`,
                );
              }
            }
          }
        ...
        if (fileHash) {
          console.log(
            `    fileHash (content): ${fileHash.old.substring(0, 12)}... → ${fileHash.new.substring(0, 12)}...`,
          );
    ...
    if (bundleChanged.length > 0) {
      ...
      bundleChanged.forEach(({ entryId, oldHash, newHash }) => {
        console.log(`  ~ ${entryId}`);
        console.log(
          `    bundleHash (bundle): ${oldHash.substring(0, 12)}... → ${newHash.substring(0, 12)}...`,
        );
        console.log(
          `      ↳ Detects: bundle structure changes (components added/removed, dependency graph)`,
        );
      });
    }
