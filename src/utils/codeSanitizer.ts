...
function getSecretMatchesForFile(
  ...
  return report.matches.filter((match) => {
    ...
    if (
      normalizedReportRoot !== normalizedCurrentRoot &&
      !isAbsolute(match.file)
    ) {
      ...
      try {
        ...
        if (
          normalizePathForComparison(relativeFromReport) ===
          normalizePathForComparison(relativeFromCurrent)
        ) {
          return true;
        }
      } catch {
...
function sanitizeLine(line: string, matches: SecretMatch[]): string {
  ...
  for (const match of sortedMatches) {
    ...
    if (match.type === 'Database URL with Credentials') {
      ...
      const dbUrlMatch = match.snippet.match(
      ...
      if (dbUrlMatch) {
        ...
        const fullUrlMatch = sanitized.match(
          /(['"`])((?:postgres|mysql|mongodb):\/\/[^:]+:)([^@]+)(@[^'"`]+)\1/i,
        );
        if (fullUrlMatch) {
    ...
    const unquotedValueMatch = match.snippet.match(
      /[=:]\s+([a-zA-Z0-9_-]{16,})/,
    );
    ...
    if (longStringMatch) {
      ...
      if (
        potentialSecret.length >= 16 &&
        sanitized.includes(potentialSecret) &&
        !sanitized.includes('PRIVATE_DATA')
      ) {
